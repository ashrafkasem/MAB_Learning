...
from MAP_core.policies.ThompsonSampling import ThompsonSampling
...
if __name__ == '__main__':
    ...
    f = open(f"standard_ts_results_{similar_arms}_{best_arms}.tsv", "w+")

    # Create 1 round of 5000 simulations
    algo = ThompsonSampling(counts=[], values=[], a=[], b=[])
    ...
    df = pd.read_csv(f"standard_ts_results_{similar_arms}_{best_arms}.tsv", sep = "\t", header = None, names = ["simulation_num", "step", "chosen_arm", "reward", "cum_reward"])
    ...
    chart = alt.Chart(df_chose_correctly).mark_line().encode(
        ...
        title = f"TS: Mean Rate of Choosing Best Arm from 5000 Simulations. {n_best_arms+n_similar_arms} Arms = [{n_similar_arms} x {similar_arms}, {n_best_arms} x {best_arms}]",
    )
    ...
    chart2 = alt.Chart(df_cumreward).mark_line().encode(
        ...
        title = f"TS: Mean Cumulative Reward from 5000 Simulations. {n_best_arms+n_similar_arms} Arms = [{n_similar_arms} x {similar_arms}, {n_best_arms} x {best_arms}]",
        )
    ...
    chart3 = alt.Chart(df_cumreward).mark_line().encode(
        ...
        title = f"TS: Mean Cumulative Regret from 5000 Simulations. {n_best_arms+n_similar_arms} Arms = [{n_similar_arms} x {similar_arms}, {n_best_arms} x {best_arms}]",
    )
